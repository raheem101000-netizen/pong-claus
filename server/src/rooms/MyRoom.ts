...
export class MyRoom extends Room {
  ...
  onJoin(client: Client) {
    if (!this.state.p1.sessionId) this.state.p1.sessionId = client.sessionId;
    else if (!this.state.p2.sessionId) this.state.p2.sessionId = client.sessionId;
  ...
  afterScore(towardP1: boolean) {
    if (this.state.p1.score >= WIN_SCORE) { this.state.winner = "p1"; this.state.playing = false; return; }
    if (this.state.p2.score >= WIN_SCORE) { this.state.winner = "p2"; this.state.playing = false; return; }
